...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='robmlm', help='which model rob| robl')
    ...
    parser.add_argument('--cuda', type=int, default=1, metavar='C', help='cuda device')
    args = parser.parse_args()

    print(args)
    
    model_family = {'rob': 'roberta', 'robl': 'roberta', 'robmlm': 'robertaMLM'}
    model_id = {'rob': 'roberta-base', 'robl': 'roberta-large', 'robmlm': 'robertaMLM'}
    model_exact_id = {'rob': 'roberta-base', 'robl': 'roberta-large', 'robmlm': 'roberta-base'}
    ...
    if context == False:
        ...
        x_train = pd.read_csv('data/processed_data' + '/dailydialog_train.csv', delimiter="\t")
        x_valid = pd.read_csv('data/processed_data' + '/dailydialog_valid.csv', delimiter="\t")
        x_test  = pd.read_csv('data/processed_data' + '/dailydialog_test.csv', delimiter="\t")
        # x_train = pd.read_csv('data/subtask2/fold' + fold + '/dailydialog_classification_test_with_context.csv')
    ...
    if fold == '1':
        num_steps = int(27672/batch_size)
    else:
