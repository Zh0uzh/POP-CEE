...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, x0=None, y0=None, x1=None, y1=None, emotion=None, position=None):
        """
        ...
        self.guid = guid
        self.text_a = eval(text_a)
        self.text_b = text_b
        self.label = label
        self.emotion = EMOTION_LIST[emotion]
        self.position = eval(position)
        # print('emotion', self.emotion)
        # print('emotion', type(self.emotion))
...
def convert_example_to_feature(
    ...
    if example.bboxes:
        ...
        if add_prefix_space and not example.text_a.startswith(" "):
            ...
            for i in range(len(example.text_a)):
                tokens_a.append(tokenizer.tokenize(example.text_a[i]))
    ...
    if example.text_b:
        ...
        for i in range(len(tokens_a)):
            ...
            if len(tokens_a[i]) > max_seq_length - special_tokens_count:
                # print('max_seq_length - special_tokens_count', max_seq_length - special_tokens_count)
                # print('len(tokens_a)', len(tokens_a))
                tokens_a[i] = (tokens_a[i][: 5] + tokens_a[i][(len(tokens_a[i]) - (max_seq_length - special_tokens_count) + 5) :])
                # print('tokens_a', tokens_a)
                # print('len(tokens_a)after', len(tokens_a))
                if example.bboxes:
    ...
    segment_ids = []
    for i in range(len(tokens_a)):
        tokens.append(tokens_a[i] + [sep_token])
        segment_ids.append([sequence_a_segment_id] * len(tokens[i]))
    ...
    if cls_token_at_end:
        ...
        for i in range(len(tokens)):
            tokens[i] = [cls_token] + tokens[i]
            segment_ids[i] = [cls_token_segment_id] + segment_ids[i]
            if bboxes:
    ...
    for i in range(len(input_ids)):
        input_mask.append([1 if mask_padding_with_zero else 0] * len(input_ids[i]))
    ...
    if pad_to_max_length:
        for i in range(len(input_ids)):
            ...
            if pad_on_left:
                input_ids[i] = ([pad_token] * padding_length) + input_ids[i]
                input_mask[i] = ([0 if mask_padding_with_zero else 1] * padding_length) + input_mask[i]
                segment_ids[i] = ([pad_token_segment_id] * padding_length) + segment_ids[i]
            else:
                input_ids[i] = input_ids[i] + ([pad_token] * padding_length)
                input_mask[i] = input_mask[i] + ([0 if mask_padding_with_zero else 1] * padding_length)
                segment_ids[i] = segment_ids[i] + ([pad_token_segment_id] * padding_length)
                if bboxes:
    ...
    # 进行补齐（防止dataloader stack时长度不相等）
    max_prompt_num = 2
    
    if len(input_ids) < max_prompt_num:
        for i in range(max_prompt_num - len(input_ids)):
            input_ids.append([pad_token] * max_seq_length)
            input_mask.append([0] * max_seq_length)
            segment_ids.append([pad_token_segment_id] * max_seq_length)
    # print('input_ids', input_ids)
