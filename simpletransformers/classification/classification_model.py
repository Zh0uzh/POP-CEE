...
from simpletransformers.classification.prompt_utils import process_prompt, project_examples_with_prompt, prompt_outputs, position_process
...
class ClassificationModel:
    def __init__(
        ...
        **kwargs,
        ...
        MODEL_CLASSES = {
            ...
            "robertaMLM": (RobertaConfig, RobertaForMaskedLM, RobertaTokenizer),
        }
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if isinstance(train_df, str) and self.args.lazy_loading:    # isinstance：判断一个对象是否是已知类型
            # print("true")
            ...
            if "text" in train_df.columns and "labels" in train_df.columns:
                ...
                if self.args.model_type == "layoutlm":
                    ...
                    # print('here')
                    train_examples = [
                        InputExample(i, text, None, label, emotion=emotion, position=position)
                        for i, (text, label, emotion, position) in enumerate(zip(train_df["text"].astype(str), train_df["labels"], train_df["emotion"], train_df["position"]))     # astype强制转换数据类型
                    ]
                    # print('train_examples[0].text_a', train_examples[0].text_a)
                    # print('train_examples[0].label', train_examples[0].label)
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if self.args.fp16:
                    ...
                    # # print('inputs', inputs['labels'].shape)
                    # # print('self.config.vocab_size', self.config.vocab_size)
                    # mask_position = np.argwhere(inputs['input_ids'].cpu()==self.tokenizer.mask_token_id)
                    # # print('mask_position', mask_position)
                    # labels_vocab = torch.full((self.args.train_batch_size, inputs['input_ids'].shape[1]), -100)
                    # # print('labels_vocab', labels_vocab)
                    # # print('labels_vocab', labels_vocab.shape)
                    ...
                    #     #     print('labels', labels)
                    #     if labels[i] == 0:
                    #         labels_vocab[i, mask_position[1, i]] = self.prompt_labels_ids[0]
                    #     else:
                    #         labels_vocab[i, mask_position[1, i]] = self.prompt_labels_ids[1]
                    # inputs['labels'] = labels_vocab
                    # # print('new inputs', inputs)
                    # # print(inputs['labels'][0])
                    # # if self.args.model_type == "robertaMLM":
                    emotion = inputs.pop('emotion')
                    positions = inputs.pop('position')
                    ...
                    inputs['input_ids'] = inputs['input_ids'].view(args.eval_batch_size * max_prompt_num, max_seq_length)
                    inputs['attention_mask'] = inputs['attention_mask'].view(args.eval_batch_size * max_prompt_num, max_seq_length)
                    outputs = model(**inputs)
                    # print('outputs', outputs['logits'].shape)
                    # print('inputs["input_ids"]', inputs["input_ids"].shape)
                    if self.args.model_type == "robertaMLM":
                        outputs = prompt_outputs(outputs['logits'].view(args.eval_batch_size, max_prompt_num, max_seq_length, outputs['logits'].shape[-1]), inputs["input_ids"].view(args.eval_batch_size, max_prompt_num, max_seq_length), self.prompt_labels_ids, self.tokenizer, labels, emotion, positions)
    ...
    def eval_model(
        self, eval_df, multi_label=False, output_dir=None, verbose=True, silent=False, wandb_log=True, pred_file=None, **kwargs
    ):
        """
        ...
        result, model_outputs, wrong_preds = self.evaluate(
            eval_df, output_dir, multi_label=multi_label, verbose=verbose, silent=silent, wandb_log=wandb_log, pred_file=pred_file, **kwargs
        )
    ...
    def evaluate(
        self, eval_df, output_dir, multi_label=False, prefix="", verbose=True, silent=False, wandb_log=True, pred_file=None, **kwargs
    ):
        """
        ...
        if isinstance(eval_df, str) and self.args.lazy_loading:
            ...
            if "text" in eval_df.columns and "labels" in eval_df.columns:
                if self.args.model_type == "layoutlm":
                    ...
                    eval_examples = [
                        InputExample(i, text, None, label, emotion=emotion, position=position)
                        for i, (text, label, emotion, position) in enumerate(zip(eval_df["text"].astype(str), eval_df["labels"], eval_df["emotion"], eval_df["position"]))
                    ]
        ...
        for i, batch in enumerate(tqdm(eval_dataloader, disable=args.silent or silent, desc="Running Evaluation", position=0, leave=True)):
            ...
            with torch.no_grad():
                ...
                if self.args.fp16:
                    ...
                    # print('input[emotion]', inputs['emotion'])
                    # print('input[position]', inputs['position'])

                    emotion = inputs.pop('emotion')
                    positions = inputs.pop('position').cpu().numpy()
                    ...
                    # prediction = np.argmax(logits.detach().cpu(), 1)
                    # for j in range(len(inputs["labels"])):
                    #     if emotion[j] == 0:
                    #         self.total['happiness'][positions[j][0] - positions[j][1]] += 1
                    #     elif emotion[j] == 1:
                    #         self.total['surprise'][positions[j][0] - positions[j][1]] += 1
                    #     elif emotion[j] == 2:
                    #         self.total['sadness'][positions[j][0] - positions[j][1]] += 1
                    #     elif emotion[j] == 3:
                    #         self.total['anger'][positions[j][0] - positions[j][1]] += 1
                    #     elif emotion[j] == 4:
                    #         self.total['fear'][positions[j][0] - positions[j][1]] += 1
                    #     else:
                    #         self.total['disgust'][positions[j][0] - positions[j][1]] += 1
                    #     if labels[j] == 1:
                    #         if emotion[j] == 0:
                    #             self.right['happiness'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 1:
                    #             self.right['surprise'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 2:
                    #             self.right['sadness'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 3:
                    #             self.right['anger'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 4:
                    #             self.right['fear'][positions[j][0] - positions[j][1]] += 1
                    #         else:
                    #             self.right['disgust'][positions[j][0] - positions[j][1]] += 1
                    #     if prediction[j] != inputs['labels'][j]:
                    #         tokens = self.tokenizer.decode(inputs["input_ids"][j])
                    #         pred_file.write(str(emotion[j]) + '------' + str(positions[j]) + '\n')
                    #         pred_file.write(tokens + '\n')
                    #         pred_file.write('prediction: ' + str(prediction[j]) + '\n')
                    #         pred_file.write('labels: ' + str(inputs['labels'][j]) + '\n')

                    #         # print('position[j][0] - position[j][1]', position[j][0] - position[j][1])
                    #         # 区分错误种类
                    #         if emotion[j] == 0:
                    #             self.error['happiness'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 1:
                    #             self.error['surprise'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 2:
                    #             self.error['sadness'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 3:
                    #             self.error['anger'][positions[j][0] - positions[j][1]] += 1
                    #         elif emotion[j] == 4:
                    #             self.error['fear'][positions[j][0] - positions[j][1]] += 1
                    #         else:
                    #             self.error['disgust'][positions[j][0] - positions[j][1]] += 1
            ...
            # print('out_label_ids(pre)', out_label_ids)
    ...
    def load_and_cache_examples(
        self, examples, evaluate=False, no_cache=False, multi_label=False, verbose=True, silent=False, useprompt=False
    ):
        """
        ...
        # print('examples[4].text_a', examples[4].text_a)
        # print('examples[4].label', examples[4].label)

        useprompt = True
        if useprompt:
            examples, prompt_labels_ids = project_examples_with_prompt(examples, tokenizer)
            self.prompt_labels_ids = prompt_labels_ids
        ...
        if os.path.exists(cached_features_file) and (
            ...
            # print('features', features[0].input_ids)
            # print('tokenizer.decode(features[0].input_ids)', tokenizer.decode(features[0].input_ids))
        ...
        # print('all_input_ids[0]', all_input_ids[1])
        # print('all_input_ids[0]', all_input_ids[1].shape)
        ...
        # 添加emotion和position
        all_emotion = torch.tensor([f.emotion for f in examples])
        all_position = torch.tensor([f.position for f in examples])
        ...
        if self.args.model_type == "layoutlm":
            ...
            dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_emotion, all_position)
    ...
    def compute_metrics(self, preds, labels, eval_examples=None, multi_label=False, **kwargs):
        ...
        if self.args.model_type == "robertaMLM" or self.model.num_labels == 2:
    ...
    def _get_inputs_dict(self, batch):
        if isinstance(batch[0], dict):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3], "emotion": batch[4], "position": batch[5]}
